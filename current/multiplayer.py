        ...
        while True:
            if self.state == "GAME":
                if self.countdown >= 0: # decrease the countdown if required
                    self.return_info["countdown"] = round(self.countdown)
                    self.countdown -= 0.5
            ...
            try:
                ...
                if msg[0] == self.state: # only update information if the state of the client matches the host
                    ...
                    if self.state == "GAME":
                        ...
                        self.return_info["num games"] = [self.games_remaining, self.total_games] # set all the return info to send to clients
