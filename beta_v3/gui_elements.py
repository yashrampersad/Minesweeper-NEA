...
try:
    ...
    flag_source = pygame.image.load(resource_path("beta_v3/flag.png"))
    mine_source = pygame.image.load(resource_path("beta_v3/mine.png"))
...
class ProgressBar(): # to display a progress bar that can change during runtime
    ...
    def draw(self, surface, x, y, percentage):
        if percentage > 1:
            percentage = 1
        pygame.draw.rect(surface, self.outline_colour, (x, y, round(self.max_width), round(PROGRESS_BAR_HEIGHT*self.scale))) # draw outline
        pygame.draw.rect(surface, self.colour, (x, y, round(self.max_width*percentage), round(PROGRESS_BAR_HEIGHT*self.scale))) # draw progress using the given float value for the percentage
